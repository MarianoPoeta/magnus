...
const Dashboard: React.FC = () => {
  ...
  const urgentTasks = useMemo(() => {
    ...
    return tasks
      .filter(t => {
        ...
        if (t.assignedToRole !== (currentUser as any).role && (currentUser as any).role !== 'admin') return false;
  ...
  const getRoleWorkflow = () => {
    const role = (currentUser as any)?.role || 'sales';
    switch (role) {
      case 'admin':
        return {
  ...
  // Guard while user is being rehydrated (must be AFTER hooks)
  if (!currentUser) {
    return (
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          <Badge variant="secondary" className="capitalize">
            {((currentUser as any)?.role) === 'admin' ? 'Administrador' : 
             ((currentUser as any)?.role) === 'sales' ? 'Ventas' : 'Principal'}
          </Badge>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <Card className="border-0 shadow-lg">
              ...
              <CardContent>
                <div className="space-y-4">
                  {tasks
                    .filter(t => !currentUser ? false : (t.assignedToRole === (currentUser as any).role || (currentUser as any).role === 'admin'))
                    .slice(0, 5)
                    .map((task) => {
