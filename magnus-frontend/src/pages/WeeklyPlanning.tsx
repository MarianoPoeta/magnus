...
const WeeklyPlanning: React.FC = () => {
  ...
  const [weeklyShoppingList, setWeeklyShoppingList] = useState<ShoppingItem[]>([]);
  ...
  // Generate enhanced shopping list using service
  useEffect(() => {
    const enhancedShoppingList = ShoppingListService.generateWeeklyShoppingList(
      budgets,
      tasks,
      weekStart,
      weekEnd
    );
    setWeeklyShoppingList(enhancedShoppingList);
  }, [budgets, tasks, weekStart, weekEnd]);
          ...
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-slate-900">{weeklyShoppingList.length}</p>
              </div>
            ...
            <CardContent>
              {weeklyShoppingList.length === 0 ? (
                ...
                <div className="space-y-4">
                  {Object.entries(
                    ShoppingListService.groupByCategory(weeklyShoppingList)
                  ).map(([category, items]) => (
                    <div key={category}>
                        ...
                        {items.map((item) => (
                          <div key={item.id} className="flex items-center justify-between p-3 bg-slate-50 rounded-lg">
                            <div>
                              ...
                              <p className="text-sm text-slate-600">Para: {item.clientNames.join(', ')}</p>
                              {item.notes && (
                            ...
                            <div className="flex items-center gap-2">
                              ...
                              {item.isPurchased ? (
                                <Badge className="bg-green-100 text-green-800">Comprado</Badge>
                              ) : (
